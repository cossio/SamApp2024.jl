...
_hallmark_sites = SamApp2024.hallmark_sites_20230507

# ╔═╡ f40c1495-a22e-45d5-9494-caa4872374d6
ss_bps, ss_nps, ss_pks = SamApp2024.RF00162_sites_paired()
...
dms_aptamer_rbm_energies = [ismissing(seq) ? missing : RBMs.free_energy(SamApp2024.rbm2022(), SamApp2024.onehot(LongRNA{4}(seq))) for seq = dms_data.aligned_sequence];
...
md"# New Figure 5 (natural examples)"
...
let fig = Makie.Figure()
	...
	for (n_ex, ax_R, ax_Δ) = zip(natural_examples, axes_shape_reactivity_profile_natural_examples, axes_shape_reactivity_diff_natural_examples)
		...
		#Makie.scatter!(ax_Δ, SamApp2024.hallmark_sites_20230507, -1.9one.(SamApp2024.hallmark_sites_20230507), markersize=7, color=:black, marker=:utriangle)
		Makie.scatter!(ax_Δ, SamApp2024.hallmark_sites_20230507, fill(minimum([x for x = shape_R_sam - shape_R_mg if !isnan(x)]), length(SamApp2024.hallmark_sites_20230507)); markersize=7, color=:black, marker=:utriangle)
		for ax = (ax_Δ, ax_R)
...
99 ∈ SamApp2024.hallmark_sites_20230507

# ╔═╡ e3300392-64b4-439c-9373-cbbf82e7083b
md"# New Figure 6 (RBM examples)"
...
let fig = Makie.Figure()
	...
	for (n_ex, ax_R, ax_Δ) = zip(RBM_examples, axes_shape_reactivity_profile_RBM_examples, axes_shape_reactivity_diff_RBM_examples)
		...
		Makie.scatter!(ax_Δ, SamApp2024.hallmark_sites_20230507, fill(minimum([x for x = _R_sam - _R_mg if !isnan(x)]), length(SamApp2024.hallmark_sites_20230507)), markersize=7, color=:black, marker=:utriangle)
	...
	#Makie.scatter!(ax_shape_avg_CM, SamApp2024.hallmark_sites_20230507, -2.0one.(SamApp2024.hallmark_sites_20230507), markersize=7, color=:black, marker=:utriangle)
...
# ╔═╡ 82da621a-806c-430b-a1e4-a013206b83ae
md"# Below"

# ╔═╡ f1522b80-26bf-4c6a-b952-be54ee4bd222
let fig = Makie.Figure()
	_xs = 1:108
	...
	Makie.band!(ax, _xs, (ΔR_dms_sam_avg_natural_no0std - ΔR_dms_sam_std_natural/2)[_xs], (ΔR_dms_sam_avg_natural_no0std + ΔR_dms_sam_std_natural/2)[_xs]; color=(:gray, 0.25))
	Makie.band!(ax, _xs, (ΔR_dms_sam_avg_rbmlo_no0std - ΔR_dms_sam_std_rbmlo/2)[_xs], (ΔR_dms_sam_avg_rbmlo_no0std + ΔR_dms_sam_std_rbmlo/2)[_xs], color=(:blue, 0.25))
	Makie.scatterlines!(ax, _xs, ΔR_dms_sam_avg_natural_no0std; linewidth=0.5, markersize=5, color=:black, label="Natural")
	Makie.scatterlines!(ax, _xs, ΔR_dms_sam_avg_rbmlo_no0std, linewidth=0.5, markersize=5, color=:blue, label="RBM (RBMscore>300)")
	...
	Makie.band!(ax, _xs, (ΔR_dms_sam_avg_natural_no0std - ΔR_dms_sam_std_natural/2)[_xs], (ΔR_dms_sam_avg_natural_no0std + ΔR_dms_sam_std_natural/2)[_xs], color=(:gray, 0.25))
	Makie.band!(ax, _xs, (ΔR_dms_sam_avg_rbmhi - ΔR_dms_sam_std_rbmhi/2)[_xs], (ΔR_dms_sam_avg_rbmhi + ΔR_dms_sam_std_rbmhi/2)[_xs], color=(:red, 0.25))
	Makie.scatterlines!(ax, _xs, ΔR_dms_sam_avg_natural_no0std; markersize=5, linewidth=0.5, color=:black, label="Natural")
	Makie.scatterlines!(ax, _xs, ΔR_dms_sam_avg_rbmhi_no0std; markersize=5, linewidth=0.5, color=:red, label="RBM (RBMscore<300)")
	...
	Makie.barplot!(ax_AC, dms_aptamers_AC_content; width=0.5, color=:black)
	Makie.xlims!(0, 109)
	Makie.hidespines!(ax_AC, :t, :r)
